...
class ApiBaseUrlStore {
  static const _apiBaseUrlKey = 'desktop_api_base_url_v2';
  static const _apiBaseUrlOverrideKey = 'desktop_api_base_url_override_v1';
  static const _adminDefaultBaseUrlKey =
  ...
  Future<String> load() async {
    ...
    if (normalizedAdmin != null &&
        !_shouldFallbackToDefault(normalizedAdmin) &&
        !_shouldDiscardLegacyHostedValue(normalizedAdmin)) {
      return normalizedAdmin;
    }
    return normalized ?? AppConfig.defaultApiBaseUrl;
  ...
  static bool _shouldDiscardLegacyHostedValue(String? savedValue) {
    if (savedValue == null) {
      return false;
    }
    final savedUri = Uri.tryParse(savedValue);
    final legacyUri = Uri.tryParse(AppConfig.legacyHostedApiBaseUrl);
    final defaultUri = Uri.tryParse(AppConfig.defaultApiBaseUrl);
    if (savedUri == null || legacyUri == null || defaultUri == null) {
      return false;
    }
    final savedHost = savedUri.host.toLowerCase();
    final legacyHost = legacyUri.host.toLowerCase();
    final defaultHost = defaultUri.host.toLowerCase();
    return savedHost == legacyHost && defaultHost != legacyHost;
  }
}
