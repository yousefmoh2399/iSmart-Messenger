...
class PdfBuilderService {
  Future<Uint8List> buildPdf({
    ...
    return Isolate.run(() => _buildPdfBytes(normalizedPaths, paperSizes, applyEnhancement));
  }
}
...
Future<Uint8List> _buildPdfBytes(
  ...
  debugPrint('PdfBuilderService: Starting PDF build. Total pages: \${imagePaths.length}');
  ...
  for (var index = 0; index < imagePaths.length; index++) {
    ...
    switch (paperSize) {
      case DocumentPaperSize.idCard:
        ...
        imageWidget = pw.ConstrainedBox(
          constraints: pw.BoxConstraints(
            maxWidth: cardWidthMm * PdfPageFormat.mm,
          ),
      ...
      case DocumentPaperSize.receipt:
        imageWidget = pw.ConstrainedBox(
          constraints: pw.BoxConstraints(
            maxWidth: 80.0 * PdfPageFormat.mm,
          ),
          child: pw.Image(image, fit: pw.BoxFit.contain),
        );
        break;
...
Future<_PreparedPdfImage> _safePrepareImage(
  ...
  if (applyEnhancement) {
    try {
      ...
      final enhancedBytes = await ImageProcessingService.enhanceDocument(
        finalBytes,
      );
...
''';
  ...
  await File('lib/features/scanner/data/pdf_builder_service.dart').writeAsString(content);
  print('Done rewriting pdf builder.');
}
